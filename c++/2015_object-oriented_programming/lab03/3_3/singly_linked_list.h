...
template <typename T>
class SinglyLinkedList: public List<T>, public IComparable
{
public:
    SinglyLinkedList(): List<T>(), head(new Element(0, nullptr)), lastElement(head) {}
    //! Read a new list from input
    SinglyLinkedList(std::ifstream &input);
    ~SinglyLinkedList();
    int len() const;
    void print() const;
    void append(T value);
...
template <typename T>
SinglyLinkedList<T>::SinglyLinkedList(std::ifstream &input) : SinglyLinkedList<T>()
{
    T number = 0;
    while ((input.peek() != '\n') && (input >> number))
        append(number);
